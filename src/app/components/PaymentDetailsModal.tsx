...
import { Box, Button, Modal, Typography, TextField, Card, CardContent, Alert } from '@mui/material';
...
const PaymentDetailsModal = ({ open, handleClose, documentId, totalPrice }) => {
    ...
    useEffect(() => {
        const fetchDocument = async () => {
            try {
                const response = await axios.get(`http://127.0.0.1:3001/api/documents/${documentId}`, {
                    headers: {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            await axios.post(`http://127.0.0.1:3001/api/documents/${documentId}/upload-receipt`, formData, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
        ...
        >
            <Box sx={style}>
                ...
                <Typography id="payment-details-modal-description" sx={{ mt: 2 }}>
                    Please transfer the amount to the following account number and upload the payment receipt.
                </Typography>
